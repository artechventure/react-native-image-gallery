...
export default class ViewPager extends PureComponent {
    ...
    createScroller() {
      return new Scroller(true, (dx, dy, scroller) => {
        if (dx === 0 && dy === 0 && scroller.isFinished()) {
          ...
          if (typeof (this.innerFlatList?.scrollToOffset) === 'function') {
                  this.innerFlatList?.scrollToOffset({ offset: curX, animated: false });
          }
    ...
    scrollToPage(page, immediate) {
      ...
      if (immediate) {
        InteractionManager.runAfterInteractions(() => {
          this.scroller.startScroll(this.scroller.getCurrX(), 0, finalX - this.scroller.getCurrX(), 0, 0);
                this.innerFlatList?.scrollToOffset({offset: finalX, animated: false});
    ...
    renderRow({ item, index }) {
      ...
      if (this.props.pageMargin > 0 && index > 0) {
        ...
        // The ListView seems to calculate a wrong width for children screen with margin.
        return (
          <View style={{
